// Thesis scope map — RED-FRAME VARIANT of scope-map.fig.mjs: identical layout,
// plus a red outline around THE THESIS + OUTPUTS zones (harness, models, C1,
// C3, code release, thesis document) so the GIVEN INPUTS column visibly sits
// OUTSIDE the thesis's own work. The frame is a fill-less red node drawn first,
// so every real node and edge paints over it; the anonymise->harness feed
// crossing the border is the point (given material flows in).
// Keep node/edge changes in sync with scope-map.fig.mjs — this variant only
// adds the frame.
// Build:  NODE_PATH=/opt/node22/lib/node_modules node figure/build/build_figures.mjs figure/diagrams/scope/scope-map/scope-map-red.fig.mjs
...
export default {
  ...
  nodes: [
    // ---- the red scope frame (drawn first, everything else paints over it) ----
    { id: 'frame', x: 875, y: 78, w: 1110, h: 647, color: 'red', fill: 'none' },
