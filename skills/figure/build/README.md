...
Rendered images (`.svg` + `.png`) are written to `$XDG_CACHE_HOME/generated/<name>/`
(default `~/.cache/generated/<name>/`; override with the `FIGURE_OUT_DIR` env var) — **not** next to the spec — so this skill's
...
`~/.cache/generated/<name>/`, not here — see the build section.) Create
